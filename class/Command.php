...
class Command {
    ...
    {
        while (true)
        { // Boucle infinie pour accepter plusieurs commandes
            $line = trim(readline("Entrez votre commande : "));
            if ($line === 'list')
            { // Commande pour lister tous les contacts
                $contacts = new \Cyprien\Contact();
            ...
            } elseif (preg_match('/^detail(?:\s+(\d+))?$/', $line, $matches))
            {
                ...
                $contact = new \Cyprien\Contact();
                $contactData = $contact->toString($id);
                echo $contactData . "\n";
            } elseif ($line === 'create')
            { // Commande pour créer un nouveau contact
                ...
                $newContact = new \Cyprien\ContactManager();
            ...
            { // Commande pour supprimer un contact par id
                $id = isset($matches[1]) && $matches[1] !== '' ? $matches[1] : trim(readline("Entrez l'id du contact : "));
                if ($id === '')
                ...
                $contact = new \Cyprien\ContactManager();
